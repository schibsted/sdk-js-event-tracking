...
Activity.prototype.initIds = function(callback) {
    ...
    this.user.getUserId(function(err, idObj) {
        if (err) {
            debug('Failed to fetch ids from CIS', err);

            delete self.visitorId;
            delete self.envId;
        } else {
            if (!self.visitorId) {
...
Activity.prototype.addUserId = function(object) {
    ...
    // Checks if the visitorId is not yet in URN format.
    ...
    // Checks if the envId is not yet in URN format.
    ...
    // Falsy check on userId, checks if 'undefined' is present in any part of the string.
